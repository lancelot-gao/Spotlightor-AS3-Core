package com.spotlightor.utils 
{
	...
	public class MathUtils
	{
		 ...
		 * @return
		 */
		public static function bounds(value:Number, min:Number, max:Number):Number { return Math.min(max, Math.max(value, min)); }
		...
		public static function maxAbs(value0:Number, ...moreValues):Number {
			...
			for each (var value:Number in moreValues) result = Math.max(result, Math.abs(value));
			return result;
		}
		...
		public static function minAbs(value0:Number, ...moreValues):Number {
			var result:Number = Math.abs(value0);
			for each (var value:Number in moreValues) result = Math.min(result, Math.abs(value));
			return result;
		}
		...
		public static function normolizeRotationInCircle(rot:Number):Number {
			rot %= 360;
			if (rot > 180) return rot - 360;
			else if (rot < -180) return rot + 360;
			return rot;
		}
